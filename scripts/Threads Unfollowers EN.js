(async function () {
    ...
    if (!HOST.includes(atob('dGhyZWFkcw=='))) {
        // "⚠️ This system only works on threads.com."
    ...
    const MODES = {
        safe: { name: 'Safe Mode', minDelay: 2.5, maxDelay: 5.0, minBatch: 8, maxBatch: 15, minBreak: 15, maxBreak: 30, limit: 100, color: '#4ade80' },
        normal: { name: 'Normal Mode', minDelay: 1.5, maxDelay: 3.0, minBatch: 12, maxBatch: 22, minBreak: 10, maxBreak: 20, limit: 150, color: '#60a5fa' },
        aggressive: { name: 'Aggressive Mode', minDelay: 0.8, maxDelay: 1.5, minBatch: 20, maxBatch: 35, minBreak: 5, maxBreak: 10, limit: 300, color: '#f87171' }
    };
    ...
    const getForceOverlayHTML = () => `
        <div class="tuf-force-content-wrap">
            ...
            <div class="tuf-text-main" style="margin-top:4px; margin-bottom:6px; font-size:16px;">Last Step to Use the System</div>
            <div class="tuf-text-sub" style="margin-top:0px; margin-bottom:18px; font-size:13px; line-height:1.4;">You must follow the creator to view the list and start the process.</div>
    ...
    <div id="tuf-modal">
        ...
        <div id="tuf-settings-overlay">
            ...
            <div style="background:rgba(255,255,255,0.03); color:#aaa; padding:10px 14px; border-radius:10px; font-size:12px; margin-bottom:15px; border:1px solid rgba(255,255,255,0.05); line-height:1.4; flex-shrink:0;">
                ⚠️ Operations are optimized with human-like delays. The user is responsible for any restrictions that may arise from API limits.
            </div>
        ...
        <div id="tuf-start-screen">
            ...
            <div class="tuf-text-sub" style="margin-bottom:32px;">Click the button below to find your non-followers and start the process.</div>
            <button id="tuf-btn-start-scan" class="tuf-primary-btn" style="max-width:240px;margin-bottom:0;margin-left:auto;margin-right:auto;">Start Scan</button>
            <div class="tuf-minimal-notice">⚠️ The user assumes all responsibility for account restrictions due to API limits.</div>
        </div>
        ...
        <div id="tuf-main-wrapper">
            <div id="tuf-main-content">
                <div id="tuf-tools">
                    ...
                    <div id="tuf-search-container"><svg id="tuf-search-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg><input type="text" id="tuf-search" placeholder="Search users..."></div>
    ...
    const updateBadgeUI = () => {
        ...
        if (s.mode !== 'custom' && MODES[s.mode]) {
            ...
            if (s.minDelay !== m.minDelay || s.maxDelay !== m.maxDelay || s.minBatch !== m.minBatch || s.maxBatch !== m.maxBatch || s.minBreak !== m.minBreak || s.maxBreak !== m.maxBreak || s.limit !== m.limit) {
                s.mode = 'custom'; modeData = { name: 'Custom Mode', color: '#a78bfa' };
            }
        }
    ...
    $('tuf-settings-save').onclick = () => {
        ...
        updateBadgeUI(); $('tuf-settings-overlay').style.display = 'none'; UI.msg('Settings saved successfully.');
    };
    ...
    const apiCall = async (endpoint, retries = 5, isSilent = false) => {
        for (let i = 0; i < retries; i++) {
            try {
                ...
                if (res.status === 429) { const w = 10 * (i + 1); if (!isSilent) UI.load('Rate Limit Exceeded', `Taking a safety break... (${w} sec)`); await delay(w * 1000); if (i === retries - 1) throw new Error("RATE_LIMIT"); continue; }
    ...
    $('tuf-btn-export').onclick = () => {
        ...
        a.download = `threads_non_followers_${new Date().getTime()}.csv`; document.body.appendChild(a); a.click(); document.body.removeChild(a);
    };
    ...
    const generateRowHTML = (u) => {
        ...
        return `<div class="tuf-row ${is}" data-id="${u.id}"><img src="${escapeHtml(u.pic) || dp}" class="tuf-avatar" onerror="this.src='${dp}'" onclick="window.open('https://www.threads.com/@${un}','_blank'); event.stopPropagation();"><div class="tuf-info"><a href="https://www.threads.com/@${un}" target="_blank" class="tuf-username" onclick="event.stopPropagation();">${un} ${u.verified ? vs : ''}</a><span class="tuf-subtext" id="sub-${u.id}">${fn}</span></div><button class="tuf-star-btn ${isW}" data-star="${u.id}" title="Add/Remove from Whitelist (Won't be unfollowed)">${starSvg}</button><div class="tuf-cb-container"><svg viewBox="0 0 14 10"><polyline points="1.5 5.5 5 9 12.5 1.5"></polyline></svg></div></div>`;
    };
    ...
    const attachRowEvents = (container) => {
        container.querySelectorAll('.tuf-row:not(.event-attached)').forEach(r => {
            ...
            r.onclick = (e) => {
                ...
                if (state.whitelist.has(i)) { alert("This user is on the whitelist (⭐). Remove the star first to select them."); return; }
    ...
    const renderList = (reset = true) => {
        ...
        if (reset) {
            ...
            if (!state.filteredUsers.length) { listEl.innerHTML = state.nonFollowers.length === 0 ? `<div class="tuf-empty">Awesome!<br><br>Everyone you follow follows you back.</div>` : `<div class="tuf-empty">No users found matching your search.</div>`; return; }
            listEl.innerHTML = '';
        }
        ...
        state.renderPage++;
        if (state.renderPage * state.RENDER_CHUNK < state.filteredUsers.length) listEl.insertAdjacentHTML('beforeend', '<div class="tuf-loading-more">Loading more...</div>');
    };
    ...
    $('tuf-btn-unfollow').onclick = async () => {
        ...
        try {
            ...
            if (state.myUsername !== CONFIG.CREATOR) { const j = await isFollowingCreator(state.creatorId, false); if (!j) { state.isUnlocked = false; $('tuf-main-content').classList.add('tuf-blurred'); $('tuf-overlay-force').style.display = 'flex'; UI.msg('Security Breach: Unfollowed the creator.'); return; } }
            
            const limit = state.settings.limit; let processIds = Array.from(state.selectedIds);
            if (processIds.length > limit) { if(!confirm(`⚠️ SECURITY WARNING ⚠️\n\nAccording to your settings, you can only unfollow a maximum of ${limit} users at a time. However, you selected ${processIds.length} users.\n\nOnly the first ${limit} users will be unfollowed. Do you want to continue?`)) return; processIds = processIds.slice(0, limit); } else if (!confirm(`Are you sure you want to unfollow ${processIds.length} users?`)) return;
            ...
            for (const i of processIds) {
                if (state.isStopped) { UI.msg('Process stopped by user.'); break; }
                d++; actionsInCurrentBatch++; UI.msg(`Processing... (${d}/${processIds.length})`); UI.prog((d / processIds.length) * 100);
                const st = $(`sub-${i}`); const r = document.querySelector(`.tuf-row[data-id="${i}"]`);
                if (r) r.classList.add('processing'); if (st) { st.style.color = '#f3f5f7'; st.textContent = 'Unfollowing...'; r.scrollIntoView({ behavior: 'smooth', block: 'center' }); }
                ...
                if (d < processIds.length && !state.isStopped) {
                    if (actionsInCurrentBatch >= currentBatchTarget) { let breakTime = randInt(s.minBreak, s.maxBreak); UI.msg(`Human-like break (${breakTime}s)...`); await delay(breakTime * 1000); actionsInCurrentBatch = 0; currentBatchTarget = randInt(s.minBatch, s.maxBatch); } 
            ...
            UI.msg('Process Completed. Returning to the list...'); await delay(1500); UI.msg('Data is ready.'); UI.prog(0); renderList(true); updateUIState();
    ...
    const attachForceFollowEvents = () => {
        ...
        const unlockSystem = () => { state.isUnlocked = true; bf.innerHTML = 'Unlocked'; bf.style.background = '#4ade80'; bf.style.color = '#000'; setTimeout(() => { const o = $('tuf-overlay-force'); if(o) o.style.display = 'none'; const m = $('tuf-main-content'); if(m) m.classList.remove('tuf-blurred'); updateUIState(); }, 800); };
        ...
        bf.onclick = async () => {
            ...
            if (!state.creatorId) {
                ed.innerHTML = 'System Error: Creator ID not found. Please refresh the page.'; ed.style.display = 'block'; bf.innerHTML = 'Try Again'; bf.disabled = false; return;
            }
            ...
            im = true;
            ed.innerHTML = 'Follow confirmation failed. Please click the <b>"Follow Manually"</b> button to follow and verify again.'; ed.style.display = 'block';
            bf.innerHTML = 'I Followed, Verify'; bf.disabled = false; bm.style.display = 'flex'; bm.onclick = () => { window.open(`https://www.threads.com/@${CONFIG.CREATOR}`, '_blank'); };
        };
    };
    ...
    $('tuf-btn-start-scan').onclick = async () => {
        ...
        try {
            UI.load('Authenticating...', 'Establishing secure connection'); await Promise.all([fetchIdentity(), fetchCreatorId()]);
            if (!state.myId || !state.myUsername) { UI.load('Connection Error', 'Failed to fetch user data. Make sure you are logged into your account.'); $('tuf-progress-fill').style.background = '#f87171'; if (!$('tuf-err-btn')) $('tuf-screen-loading').insertAdjacentHTML('beforeend', `<button id="tuf-err-btn" class="tuf-primary-btn tuf-close-btn" style="margin-top:20px;width:auto;">Close Interface</button>`); return; }
            const p = window.location.pathname; if (p !== `/@${state.myUsername}` && !p.startsWith(`/@${state.myUsername}/`)) { UI.load('Security Protocol', `For operational security, you must be on your own profile. Redirecting to your profile (@${state.myUsername})...`); $('tuf-progress-fill').style.background = '#eab308'; setTimeout(() => { window.location.href = `/@${state.myUsername}`; }, 2500); return; }
            
            UI.load('Fetching your followings...', 'This might take a while depending on the number of users');
            const fw = await fetchList(state.myId, 'following', n => { UI.load(`Following: ${n} users...`, 'Please do not close the tab'); UI.prog(Math.min(45, n / 10)); });
            
            UI.load('Fetching your followers...', 'Almost there...');
            const fr = await fetchList(state.myId, 'followers', n => { UI.load(`Followers: ${n} users...`, 'Comparing data'); UI.prog(50 + Math.min(50, n / 10)); });
            ...
            $('tuf-screen-loading').innerHTML = `<div style="font-size:44px;margin-bottom:15px;">❌</div><div class="tuf-text-main" style="color:#f87171;font-size:18px;">System Error</div><div class="tuf-text-sub" style="margin-bottom: 20px; font-size:14px;">${e.message === "LIST_EMPTY" ? "Could not fetch account data (0 Followers/Following)." : e.message === "INVALID_JSON" ? "Server returned an invalid response. Please refresh the page." : e.message === "TIMEOUT" ? "Connection timed out. Check your internet connection." : e.message === "RATE_LIMIT" ? "Too many requests made. Please wait a few minutes." : e.message}</div><button class="tuf-primary-btn tuf-close-btn" style="width:auto;">Close Interface</button>`;
