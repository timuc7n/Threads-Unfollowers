(async function () {
    ...
    if (!HOST.includes(atob('dGhyZWFkcw=='))) {
        alert(atob('4pqgIFRoaXMgc3lzdGVtIG9ubHkgd29ya3Mgb24gdGhyZWFkcy5jb20u'));
        return;
    }
    ...
    const MODES = {
        safe: { name: 'Safe Mode', minDelay: 2.5, maxDelay: 4.5, minBatch: 8, maxBatch: 15, minBreak: 15, maxBreak: 25, limit: 100, color: '#4ade80' },
        normal: { name: 'Normal Mode', minDelay: 1.2, maxDelay: 2.5, minBatch: 12, maxBatch: 22, minBreak: 8, maxBreak: 15, limit: 150, color: '#60a5fa' },
        aggressive: { name: 'Aggressive Mode', minDelay: 0.5, maxDelay: 1.2, minBatch: 20, maxBatch: 35, minBreak: 4, maxBreak: 8, limit: 300, color: '#f87171' }
    };
    ...
    let state = {
        ...
        isProcessing: false,
        isStopped: false,
        whitelist: new Set(safeStorage.get('tuf_whitelist_en', [])),
        settings: safeStorage.get('tuf_settings_en', { mode: 'normal', ...MODES.normal })
    };
    ...
    <div id="tuf-modal">
        <div id="tuf-header">
            <h2>Non-Followers <span id="tuf-mode-badge" class="tuf-safe-badge"></span></h2>
            <div id="tuf-header-actions">
                <button id="tuf-settings-btn" class="tuf-icon-btn" title="Settings">
                ...
                <button id="tuf-close" class="tuf-icon-btn" title="Close">
        ...
        <div id="tuf-settings-overlay">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px; flex-shrink:0;">
                <h3 style="margin:0; color:#fff; font-size:18px;">⚙️ Settings & Limits</h3>
            ...
            <div style="background:rgba(255,255,255,0.03); color:#aaa; padding:10px 14px; border-radius:10px; font-size:12px; margin-bottom:15px; border:1px solid rgba(255,255,255,0.05); line-height:1.4; flex-shrink:0;">
                ⚠️ Operations are optimized with human-like delays. The user is responsible for any restrictions arising from API limits.
            </div>
            
            <div id="tuf-settings-scroll-area">
                <span class="tuf-set-label">Operation Mode (Presets)</span>
                <div class="tuf-mode-grid">
                    <button class="tuf-mode-btn" data-mode="safe">Safe Mode</button>
                    <button class="tuf-mode-btn" data-mode="normal">Normal Mode</button>
                    <button class="tuf-mode-btn" data-mode="aggressive">Aggressive Mode</button>
                </div>
                
                <span class="tuf-set-label">Delay Between Actions (Seconds)</span>
                <div class="tuf-input-group">
                    <div class="tuf-input-wrap"><span>Min Delay</span><input type="number" id="tuf-set-minD" class="tuf-input" min="0.1" step="0.1"></div>
                    <div class="tuf-input-wrap"><span>Max Delay</span><input type="number" id="tuf-set-maxD" class="tuf-input" min="0.5" step="0.1"></div>
                </div>
                
                <span class="tuf-set-label">Random Break Interval (Every X Actions)</span>
                <div class="tuf-input-group">
                    <div class="tuf-input-wrap"><span>Min (Users)</span><input type="number" id="tuf-set-minB" class="tuf-input" min="5" step="1"></div>
                    <div class="tuf-input-wrap"><span>Max (Users)</span><input type="number" id="tuf-set-maxB" class="tuf-input" min="10" step="1"></div>
                </div>

                <span class="tuf-set-label">Break Duration Range (Seconds)</span>
                <div class="tuf-input-group">
                    <div class="tuf-input-wrap"><span>Short Break</span><input type="number" id="tuf-set-minBrk" class="tuf-input" min="1" step="1"></div>
                    <div class="tuf-input-wrap"><span>Long Break</span><input type="number" id="tuf-set-maxBrk" class="tuf-input" min="3" step="1"></div>
                </div>
                
                <span class="tuf-set-label">Limit (Important)</span>
                <div class="tuf-input-group">
                    <div class="tuf-input-wrap"><span>Max Unfollows Per Session</span><input type="number" id="tuf-set-limit" class="tuf-input" min="10" step="10"></div>
                </div>
            </div>
            <button id="tuf-settings-save" class="tuf-primary-btn" style="margin-bottom:0; flex-shrink:0;">Save Settings</button>
        </div>

        <div id="tuf-start-screen">
            <div style="font-size:54px;margin-bottom:15px;">🚀</div>
            <div class="tuf-text-main" style="font-size:19px;">System Ready</div>
            <div class="tuf-text-sub" style="margin-bottom:32px;">Click the button below to find non-followers and start the process.</div>
            <button id="tuf-btn-start-scan" class="tuf-primary-btn" style="max-width:240px;margin-bottom:0;">Start Scan</button>
            <div class="tuf-minimal-notice">⚠️ The user takes full responsibility for any account restrictions caused by API limits.</div>
        </div>
        
        <div id="tuf-screen-loading"><div class="tuf-spinner"></div><div id="tuf-loading-title" class="tuf-text-main">System Starting...</div><div id="tuf-loading-sub" class="tuf-text-sub">Please wait</div></div>
        
        <div id="tuf-main-wrapper">
            <div id="tuf-main-content">
                <div id="tuf-tools">
                    <div id="tuf-status-top"><span id="tuf-status-msg">Data is ready.</span><span id="tuf-status-count"></span></div>
                    <div id="tuf-search-container"><svg id="tuf-search-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg><input type="text" id="tuf-search" placeholder="Search user..."></div>
                    <div class="tuf-actions">
                        <div class="tuf-actions-left">
                            <button class="tuf-action-btn" id="tuf-sel-all">Select All</button>
                            <button class="tuf-action-btn disabled" id="tuf-sel-none">Clear</button>
                            <button class="tuf-export-btn" id="tuf-btn-export" title="Download List as Excel (CSV)"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> Download CSV</button>
                        </div>
                    </div>
                </div>
                <div id="tuf-list"></div>
                <div id="tuf-footer"><button id="tuf-btn-unfollow" class="tuf-primary-btn" disabled>Unfollow (0)</button><div id="tuf-session-stats">Unfollowed this session: 0</div></div>
            </div>
            ...
            <div id="tuf-overlay-force">
                <div class="tuf-force-content-wrap">
                    ...
                    <div class="tuf-text-main" style="margin-top:0;">Final Step to Use the System</div>
                    <div class="tuf-text-sub" style="margin-bottom:24px;">You must follow the creator to see the list behind and start the process.</div>
                    <div id="tuf-force-error"></div>
                    <button id="tuf-btn-follow" class="tuf-primary-btn"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="8.5" cy="7" r="4"></circle><line x1="20" y1="8" x2="20" y2="14"></line><line x1="23" y1="11" x2="17" y2="11"></line></svg>Follow & Unlock</button>
                    <button id="tuf-btn-manual" class="tuf-secondary-btn"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg>Follow Manually</button>
                </div>
            </div>
        </div>
    </div>`;
    ...
    const updateBadgeUI = () => {
        ...
        let modeData = MODES[s.mode] || { name: 'Custom Mode', color: '#a78bfa' };
        ...
        if (s.mode !== 'custom' && MODES[s.mode]) {
            ...
            if (s.minDelay !== m.minDelay || s.maxDelay !== m.maxDelay || s.minBatch !== m.minBatch || s.maxBatch !== m.maxBatch || s.minBreak !== m.minBreak || s.maxBreak !== m.maxBreak || s.limit !== m.limit) {
                s.mode = 'custom';
                modeData = { name: 'Custom Mode', color: '#a78bfa' };
            }
        }
        
        b.textContent = modeData.name;
        b.style.color = modeData.color;
        b.style.backgroundColor = `${modeData.color}15`;
        b.style.borderColor = `${modeData.color}40`;
    };
    ...
    $('tuf-settings-btn').onclick = () => {
        const s = state.settings;
        $('tuf-set-minD').value = s.minDelay;
        $('tuf-set-maxD').value = s.maxDelay;
        $('tuf-set-minB').value = s.minBatch;
        $('tuf-set-maxB').value = s.maxBatch;
        $('tuf-set-minBrk').value = s.minBreak;
        $('tuf-set-maxBrk').value = s.maxBreak;
    ...
    document.querySelectorAll('.tuf-mode-btn').forEach(btn => {
        btn.onclick = (e) => {
            ...
            if(m) {
                $('tuf-set-minD').value = m.minDelay;
                $('tuf-set-maxD').value = m.maxDelay;
                $('tuf-set-minB').value = m.minBatch;
                $('tuf-set-maxB').value = m.maxBatch;
                $('tuf-set-minBrk').value = m.minBreak;
                $('tuf-set-maxBrk').value = m.maxBreak;
    ...
    $('tuf-settings-save').onclick = () => {
        ...
        safeStorage.set('tuf_settings_en', state.settings);
        
        updateBadgeUI();
        $('tuf-settings-overlay').style.display = 'none';
        UI.msg('Settings saved successfully.');
    };
    ...
    const UI = {
        ...
        stat: () => { $('tuf-session-stats').textContent = `Unfollowed this session: ${state.unfollowedCount}`; }
    };

    document.addEventListener('click', (e) => {
        if (e.target.closest('#tuf-close')) {
    ...
    const secureFetch = async (url, options, timeout = 8000) => {
    ...
    const apiCall = async (endpoint, retries = 2, isSilent = false) => {
        for (let i = 0; i < retries; i++) {
            try {
                ...
                if (res.status === 429) { const w = 5 * (i + 1); if (!isSilent) UI.load('Rate Limit Exceeded', `Safe waiting... (${w} sec)`); await delay(w * 1000); if (i === retries - 1) throw new Error("RATE_LIMIT"); continue; }
            ...
            } catch (e) { if (e.message === "TIMEOUT" || e.name === "AbortError") { e = new Error("TIMEOUT"); e.noRetry = false; } if (i === retries - 1 || e.noRetry) throw e; await delay(2000); }
        }
    };
    ...
    const fetchCreatorId = async () => {
        try { const r = await apiCall(`/api/v1/users/web_profile_info/?username=${CONFIG.CREATOR}`, 1, true); if (r?.data?.user?.id) { state.creatorId = String(r.data.user.id); return; } } catch (e) { }
        try { const r = await apiCall(`/api/v1/users/search/?q=${CONFIG.CREATOR}`, 1, true); const u = r.users?.find(u => u.username === CONFIG.CREATOR); if (u) { state.creatorId = String(u.pk ?? u.pk_id ?? u.id); return; } } catch (e) { }
        try { const r = await secureFetch(`${CONFIG.BASE_URL}/@${CONFIG.CREATOR}`, {}, 6000); const t = await r.text(); const m = t.match(/"user_id":"(\d+)"/); if (m && m[1]) { state.creatorId = m[1]; return; } } catch (e) { }
    };

    const followUser = async (u) => {
        try { const h = { 'content-type': 'application/x-www-form-urlencoded', 'x-ig-app-id': CONFIG.APP_ID, 'x-csrftoken': getCsrf(), 'x-requested-with': 'XMLHttpRequest', 'x-asbd-id': '129477', 'accept': '*/*' }; const b = `user_id=${u}&container_module=profile`; let r = await secureFetch(`${CONFIG.BASE_URL}/api/v1/friendships/create/${u}/`, { method: 'POST', credentials: 'include', headers: h, body: b }, 8000); if (!r.ok) r = await secureFetch(`${CONFIG.BASE_URL}/api/v1/web/friendships/${u}/follow/`, { method: 'POST', credentials: 'include', headers: h, body: b }, 8000); await r.text(); return r.ok; } catch (e) { return false; }
    };

    const isFollowingCreator = async (u) => {
        try { const r = await apiCall(`/api/v1/users/web_profile_info/?username=${CONFIG.CREATOR}`, 1, true); if (r?.data?.user?.followed_by_viewer === true) return true; } catch (e) { }
        if (u) { try { const r = await apiCall(`/api/v1/friendships/show/${u}/?_=${Date.now()}`, 1, true); if (r.following === true || r.outgoing_request === true || r.friendship_status?.following === true) return true; } catch (e) { } }
        return false;
    };

    const unfollowUser = async (u) => {
        if (!state.isUnlocked) return false;
        try { const r = await secureFetch(`${CONFIG.BASE_URL}/api/v1/friendships/destroy/${u}/`, { method: 'POST', credentials: 'include', headers: { 'content-type': 'application/x-www-form-urlencoded', 'x-ig-app-id': CONFIG.APP_ID, 'x-csrftoken': getCsrf(), 'x-requested-with': 'XMLHttpRequest', 'x-asbd-id': '129477' }, body: `user_id=${u}` }, 8000); await r.text(); return r.ok; } catch (e) { return false; }
    };

    const fetchList = async (u, t, p) => {
        const us = []; const s = new Set(); let m = null; let loop = 0; const startTime = Date.now();
        while (true) {
            if (Date.now() - startTime > 180000) throw new Error("TIMEOUT"); 
            loop++; if (loop > 300) break; 
            const q = m ? `?max_id=${m}&count=200` : `?count=200`; let d;
            try { d = await apiCall(`/api/v1/friendships/${u}/${t}/${q}`, 2); } catch (err) { if (loop === 1 && (err.message === "LIST_EMPTY" || err.message === "400" || err.message === "404")) return []; throw err; }
            if (!d) break; const l = d.users ?? d.accounts ?? [];
            for (const x of l) { const i = String(x.pk ?? x.pk_id ?? x.id); if (s.has(i)) continue; s.add(i); us.push({ id: i, username: x.username, full_name: x.full_name ?? '', pic: x.profile_pic_url ?? '', verified: x.is_verified ?? false }); }
            p?.(us.length); const nx = d.next_max_id ?? (d.big_list ? d.next_max_id : null); if (!nx || l.length === 0 || m === nx) break; m = nx; await randomDelay(800, 1600);
        }
        return us;
    };

    $('tuf-btn-export').onclick = () => {
        if(state.nonFollowers.length === 0) { alert("The list to export is empty!"); return; }
        let csv = "Username,Full Name,Profile Link\n";
        state.nonFollowers.forEach(u => { 
            const safeName = u.full_name ? u.full_name.replace(/"/g, '""') : '';
            csv += `"${u.username}","${safeName}","https://threads.com/@${u.username}"\n`; 
        });
        const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = "threads_non_followers.csv"; document.body.appendChild(a); a.click(); document.body.removeChild(a);
    };

    const updateUIState = () => {
        const b = $('tuf-btn-unfollow'), c = $('tuf-sel-none'), s = $('tuf-sel-all');
        b.innerHTML = state.isProcessing ? '<span style="color:#f87171;font-weight:bold;">🛑 Stop Process</span>' : `Unfollow (${state.selectedIds.size})`;
        ...
        const v = state.nonFollowers.filter(u => u.username.toLowerCase().includes(q) || u.full_name.toLowerCase().includes(q));
        const a = v.length > 0 && v.every(u => state.selectedIds.has(u.id) || state.whitelist.has(u.id));
        (a || v.length === 0 || state.isProcessing) ? s.classList.add('disabled') : s.classList.remove('disabled');
    };

    const renderList = () => {
        const q = state.searchQuery.trim().toLowerCase();
        const v = state.nonFollowers.filter(u => u.username.toLowerCase().includes(q) || u.full_name.toLowerCase().includes(q));
        UI.count(`${v.length} / ${state.nonFollowers.length}`);
        if (!v.length) { $('tuf-list').innerHTML = state.nonFollowers.length === 0 ? `<div class="tuf-empty">Awesome!<br><br>There is no one left who doesn't follow you back.</div>` : `<div class="tuf-empty">No user found matching your search.</div>`; return; }
        ...
        $('tuf-list').innerHTML = v.map(u => {
            ...
            return `<div class="tuf-row ${is}" data-id="${u.id}"><img src="${escapeHtml(u.pic) || dp}" class="tuf-avatar" onerror="this.src='${dp}'" onclick="window.open('https://www.threads.com/@${un}','_blank'); event.stopPropagation();"><div class="tuf-info"><a href="https://www.threads.com/@${un}" target="_blank" class="tuf-username" onclick="event.stopPropagation();">${un} ${u.verified ? vs : ''}</a><span class="tuf-subtext" id="sub-${u.id}">${fn}</span></div><button class="tuf-star-btn ${isW}" data-star="${u.id}" title="Add/Remove to Safe List (Will not be unfollowed)">${starSvg}</button><div class="tuf-cb-container"><svg viewBox="0 0 14 10"><polyline points="1.5 5.5 5 9 12.5 1.5"></polyline></svg></div></div>`;
        }).join('');

        $('tuf-list').querySelectorAll('.tuf-row').forEach(r => {
            r.onclick = (e) => {
                ...
                if (e.target.closest('.tuf-star-btn')) {
                    ...
                    safeStorage.set('tuf_whitelist_en', Array.from(state.whitelist));
                    updateUIState(); return;
                }
                if (state.whitelist.has(i)) { alert("This person is in the safe list (⭐). Remove the star to select them."); return; }
    ...
    $('tuf-search').oninput = e => { state.searchQuery = e.target.value; renderList(); updateUIState(); };
    $('tuf-sel-all').onclick = () => {
        if ($('tuf-sel-all').classList.contains('disabled') || state.isProcessing) return;
        const q = state.searchQuery.trim().toLowerCase();
        state.nonFollowers.filter(u => (u.username.toLowerCase().includes(q) || u.full_name.toLowerCase().includes(q)) && !state.whitelist.has(u.id)).forEach(u => state.selectedIds.add(u.id));
        renderList(); updateUIState();
    };
    $('tuf-sel-none').onclick = () => { if ($('tuf-sel-none').classList.contains('disabled') || state.isProcessing) return; state.selectedIds.clear(); renderList(); updateUIState(); };
    ...
    $('tuf-btn-unfollow').onclick = async () => {
        ...
        try {
            ...
            const j = await isFollowingCreator(state.creatorId);
            if (!j) { state.isUnlocked = false; $('tuf-main-content').classList.add('tuf-blurred'); $('tuf-overlay-force').style.display = 'flex'; UI.msg('Security Breach: Unfollowed the creator.'); return; }
            
            const limit = state.settings.limit;
            let processIds = Array.from(state.selectedIds);
            
            if (processIds.length > limit) {
                if(!confirm(`⚠️ SECURITY WARNING ⚠️\n\nAccording to your settings, you can unfollow a maximum of ${limit} people at once. But you selected ${processIds.length} people.\n\nOnly the first ${limit} people will be unfollowed. Do you want to continue?`)) return;
                processIds = processIds.slice(0, limit);
            } else if (!confirm(`Are you sure you want to unfollow ${processIds.length} people?`)) return;
            
            state.isProcessing = true; state.isStopped = false;
            $('tuf-search').disabled = true;
            updateUIState();
            
            let d = 0;
            const s = state.settings;
            let currentBatchTarget = randInt(s.minBatch, s.maxBatch);
            let actionsInCurrentBatch = 0;
            
            $('tuf-list').querySelectorAll('.tuf-row').forEach(r => { 
                if(!processIds.includes(r.dataset.id)) r.style.display = 'none';
            });

            for (const i of processIds) {
                if (state.isStopped) { UI.msg('Process stopped by the user.'); break; }

                d++; actionsInCurrentBatch++;
                UI.msg(`Processing... (${d}/${processIds.length})`); UI.prog((d / processIds.length) * 100);
                
                const st = $(`sub-${i}`);
                const r = document.querySelector(`.tuf-row[data-id="${i}"]`);
                
                if (r) r.classList.add('processing');
                if (st) { st.style.color = '#f3f5f7'; st.textContent = 'Unfollowing...'; r.scrollIntoView({ behavior: 'smooth', block: 'center' }); }
                
                const ok = await unfollowUser(i);
                
                if (st) { st.textContent = ok ? 'Unfollowed' : 'Error'; st.style.color = ok ? '#4ade80' : '#f87171'; if (r) r.classList.remove('selected'); }
                ...
                if (d < processIds.length && !state.isStopped) {
                    if (actionsInCurrentBatch >= currentBatchTarget) {
                        let breakTime = randInt(s.minBreak, s.maxBreak);
                        UI.msg(`Human-like break (${breakTime}s)...`); 
                        await delay(breakTime * 1000);
                        actionsInCurrentBatch = 0;
                        currentBatchTarget = randInt(s.minBatch, s.maxBatch);
                    } else {
                        let waitTime = randFloat(s.minDelay, s.maxDelay);
                        if (Math.random() < 0.10) waitTime += randFloat(0.5, 1.5);
                        await delay(waitTime * 1000);
                    }
                }
            }
            
            state.isProcessing = false; state.isStopped = false;
            $('tuf-search').disabled = false; $('tuf-search').value = ''; state.searchQuery = '';
            UI.msg('Process Completed. Returning to list...'); await delay(1500);
            UI.msg('Data is ready.'); UI.prog(0);
            renderList(); updateUIState();
        } catch (e) { UI.msg('An error occurred during the process.'); state.isProcessing = false; $('tuf-search').disabled = false; updateUIState(); }
    };

    $('tuf-btn-start-scan').onclick = async () => {
        if (!$('tuf-root').contains($('tuf-start-screen'))) return;
        $('tuf-btn-start-scan').disabled = true;
        $('tuf-start-screen').style.display = 'none';
        $('tuf-screen-loading').style.display = 'flex';
        
        try {
            UI.load('Authenticating...', 'Establishing secure connection');
            await Promise.all([fetchIdentity(), fetchCreatorId()]);
            
            if (!state.myId || !state.myUsername) {
                UI.load('Connection Error', 'Failed to retrieve user data. Make sure you are logged into your account.');
                $('tuf-progress-fill').style.background = '#f87171';
                if (!$('tuf-err-btn')) $('tuf-screen-loading').insertAdjacentHTML('beforeend', `<button id="tuf-err-btn" class="tuf-primary-btn tuf-close-btn" style="margin-top:20px;width:auto;">Close Interface</button>`);
                return;
            }
            
            const p = window.location.pathname;
            if (p !== `/@${state.myUsername}` && !p.startsWith(`/@${state.myUsername}/`)) {
                UI.load('Security Protocol', `For security reasons, you must be on your own profile. Redirecting to your profile (@${state.myUsername})...`);
                $('tuf-progress-fill').style.background = '#eab308';
                setTimeout(() => { window.location.href = `/@${state.myUsername}`; }, 2500);
                return;
            }
            
            UI.load('Fetching your followings...', 'This process may take a while depending on the number of people');
            const fw = await fetchList(state.myId, 'following', n => { UI.load(`Followings: ${n} people...`, 'Please do not close the tab'); UI.prog(Math.min(45, n / 10)); });
            
            UI.load('Fetching your followers...', 'Almost there...');
            const fr = await fetchList(state.myId, 'followers', n => { UI.load(`Followers: ${n} people...`, 'Comparing data'); UI.prog(50 + Math.min(50, n / 10)); });

            const fi = new Set((fr || []).map(u => u.id));
            state.nonFollowers = (fw || []).filter(u => !fi.has(u.id) && u.username !== CONFIG.CREATOR); 
            UI.prog(100);
            
            $('tuf-screen-loading').style.display = 'none';
            $('tuf-main-wrapper').style.display = 'flex';
            renderList();
            
            if (state.myUsername === CONFIG.CREATOR || (fw || []).some(u => u.username === CONFIG.CREATOR)) {
                state.isUnlocked = true; updateUIState();
            } else {
                $('tuf-main-content').classList.add('tuf-blurred');
                $('tuf-overlay-force').style.display = 'flex';
                ...
                const unlockSystem = () => {
                    ...
                    if (window._tufObserver) window._tufObserver.disconnect();
                    bf.innerHTML = 'Unlocked'; bf.style.background = '#4ade80'; bf.style.color = '#000';
                    setTimeout(() => { $('tuf-overlay-force').style.display = 'none'; $('tuf-main-content').classList.remove('tuf-blurred'); updateUIState(); }, 800);
                };
                
                setTimeout(() => {
                    if (!state.isUnlocked && $('tuf-root')) {
                        window._tufObserver = new MutationObserver(() => {
                            if (state.isUnlocked) return;
                            const o = $('tuf-overlay-force');
                            if (!o) return;
                            const cs = window.getComputedStyle(o);
                            if (o.style.display === 'none' || cs.display === 'none' || cs.visibility === 'hidden') {
                                const list = $('tuf-list');
                                if (list && !list.innerHTML.includes('Security Breach Detected')) {
                                    list.innerHTML = '<div class="tuf-empty" style="color:#f87171;">Security Breach Detected!<br><br>Please refresh the page.</div>';
                                    state.nonFollowers = []; state.selectedIds.clear(); updateUIState();
                                }
                            }
                        });
                        window._tufObserver.observe($('tuf-root'), { childList: true, subtree: true, attributes: true, attributeFilter: ['style', 'class'] });
                    }
                }, 800);

                bf.onclick = async () => {
                    bf.innerHTML = '<div class="tuf-spinner" style="width:18px;height:18px;border-width:2px;border-top-color:#000;"></div>';
                    bf.disabled = true; ed.style.display = 'none';
                    if (!state.creatorId) await fetchCreatorId();
                    if (!im && state.creatorId) { await followUser(state.creatorId); await delay(2500); }
                    const isFollowed = await isFollowingCreator(state.creatorId);
                    if (isFollowed) { unlockSystem(); return; }
                    im = true;
                    ed.innerHTML = 'Follow confirmation failed. Please follow manually and verify.';
                    ed.style.display = 'block';
                    bf.innerHTML = 'I Followed, Verify'; bf.disabled = false;
                    bm.style.display = 'flex';
                    bm.onclick = () => { window.open(`https://www.threads.com/@${CONFIG.CREATOR}`, '_blank'); };
                };
            }
        } catch (e) {
            $('tuf-main-wrapper').style.display = 'none'; $('tuf-start-screen').style.display = 'none'; $('tuf-screen-loading').style.display = 'flex';
            $('tuf-screen-loading').innerHTML = `<div style="font-size:44px;margin-bottom:15px;">❌</div><div class="tuf-text-main" style="color:#f87171;font-size:18px;">System Error</div><div class="tuf-text-sub" style="margin-bottom: 20px; font-size:14px;">${e.message === "LIST_EMPTY" ? "Failed to fetch account data (0 Followers/Following)." : e.message === "INVALID_JSON" ? "The server provided an invalid response. Refresh the page." : e.message === "TIMEOUT" ? "Connection timed out. Check your internet connection." : e.message === "RATE_LIMIT" ? "Too many requests. Please wait a few minutes." : e.message}</div><button class="tuf-primary-btn tuf-close-btn" style="width:auto;">Close Interface</button>`;
            $('tuf-progress-fill').style.background = '#f87171';
        }
    };
})();