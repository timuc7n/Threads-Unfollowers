(async function () {
    ...
    const MODES = {
        safe: { name: 'Güvenli Mod', minDelay: 2.5, maxDelay: 4.5, minBatch: 8, maxBatch: 15, minBreak: 15, maxBreak: 25, limit: 100, color: '#4ade80' },
        normal: { name: 'Normal Mod', minDelay: 1.2, maxDelay: 2.5, minBatch: 12, maxBatch: 22, minBreak: 8, maxBreak: 15, limit: 150, color: '#60a5fa' },
        aggressive: { name: 'Agresif Mod', minDelay: 0.5, maxDelay: 1.2, minBatch: 20, maxBatch: 35, minBreak: 4, maxBreak: 8, limit: 300, color: '#f87171' }
    };
    ...
    let state = {
        ...
        settings: safeStorage.get('tuf_settings_v3', { mode: 'normal', ...MODES.normal })
    };
    ...
    <div id="tuf-modal">
        ...
        <div id="tuf-start-screen">
            ...
            <button id="tuf-btn-start-scan" class="tuf-primary-btn" style="max-width:240px;margin-bottom:0;">Taramayı Başlat</button>
        ...
        <div id="tuf-main-wrapper">
            ...
            <div id="tuf-overlay-force">
                <div class="tuf-force-content-wrap">
                    ...
                    <div class="tuf-text-main" style="margin-top:0;">Sistemi Kullanmak İçin Son Adım</div>
                    <div class="tuf-text-sub" style="margin-bottom:24px;">Arkadaki listeyi görmek ve işlemlere başlamak için yapımcıyı takip etmelisin.</div>
    ...
    const updateBadgeUI = () => {
        ...
        if (s.mode !== 'custom' && MODES[s.mode]) {
            ...
            if (s.minDelay !== m.minDelay || s.maxDelay !== m.maxDelay || s.minBatch !== m.minBatch || s.maxBatch !== m.maxBatch || s.minBreak !== m.minBreak || s.maxBreak !== m.maxBreak || s.limit !== m.limit) {
                s.mode = 'custom';
                modeData = { name: 'Özel Mod', color: '#a78bfa' };
            }
        }
    ...
    $('tuf-settings-save').onclick = () => {
        ...
        safeStorage.set('tuf_settings_v3', state.settings);
        
        updateBadgeUI();
        $('tuf-settings-overlay').style.display = 'none';
        UI.msg('Ayarlar başarıyla kaydedildi.');
    };
    ...
    const apiCall = async (endpoint, retries = 2, isSilent = false) => {
        for (let i = 0; i < retries; i++) {
            try {
                ...
                if (res.status === 429) { const w = 5 * (i + 1); if (!isSilent) UI.load('Hız Sınırı Aşıldı', `Güvenli bekleme yapılıyor... (${w} sn)`); await delay(w * 1000); if (i === retries - 1) throw new Error("RATE_LIMIT"); continue; }
    ...
    $('tuf-btn-export').onclick = () => {
        ...
        const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = "threads_takip_etmeyenler.csv"; document.body.appendChild(a); a.click(); document.body.removeChild(a);
    };
    ...
    const renderList = () => {
        ...
        if (!v.length) { $('tuf-list').innerHTML = state.nonFollowers.length === 0 ? `<div class="tuf-empty">Harika!<br><br>Seni takip etmeyen kimse kalmadı.</div>` : `<div class="tuf-empty">Aramaya uygun kişi bulunamadı.</div>`; return; }
        ...
        $('tuf-list').querySelectorAll('.tuf-row').forEach(r => {
            r.onclick = (e) => {
                ...
                if (e.target.closest('.tuf-star-btn')) {
                    ...
                    safeStorage.set('tuf_whitelist', Array.from(state.whitelist));
    ...
    $('tuf-btn-unfollow').onclick = async () => {
        ...
        try {
            ...
            if (!j) { state.isUnlocked = false; $('tuf-main-content').classList.add('tuf-blurred'); $('tuf-overlay-force').style.display = 'flex'; UI.msg('Güvenlik İhlali: Takipten çıkılmış.'); return; }
            
            const limit = state.settings.limit;
            let processIds = Array.from(state.selectedIds);
            
            if (processIds.length > limit) {
                if(!confirm(`⚠️ GÜVENLİK UYARISI ⚠️\n\nAyarlarınız gereği tek seferde maksimum ${limit} kişi çıkarabilirsiniz. Ancak siz ${processIds.length} kişi seçtiniz.\n\nSadece ilk ${limit} kişi takipten çıkarılacak. Devam edilsin mi?`)) return;
                processIds = processIds.slice(0, limit);
            } else if (!confirm(`${processIds.length} kişiyi takipten çıkarmak istediğinize emin misiniz?`)) return;
            ...
            for (const i of processIds) {
                ...
                UI.msg(`İşleniyor... (${d}/${processIds.length})`); UI.prog((d / processIds.length) * 100);
                
                const st = $(`sub-${i}`);
                const r = document.querySelector(`.tuf-row[data-id="${i}"]`);
                
                if (r) r.classList.add('processing');
                if (st) { st.style.color = '#f3f5f7'; st.textContent = 'Çıkarılıyor...'; r.scrollIntoView({ behavior: 'smooth', block: 'center' }); }
                ...
                if (d < processIds.length && !state.isStopped) {
                    if (actionsInCurrentBatch >= currentBatchTarget) {
                        ...
                        UI.msg(`İnsansı mola (${breakTime}sn)...`); 
            ...
            UI.msg('İşlem Tamamlandı. Listeye dönülüyor...'); await delay(1500);
            UI.msg('Veriler hazır.'); UI.prog(0);
            renderList(); updateUIState();
    ...
    $('tuf-btn-start-scan').onclick = async () => {
        ...
        try {
            UI.load('Kimlik doğrulanıyor...', 'Güvenli bağlantı kuruluyor');
            await Promise.all([fetchIdentity(), fetchCreatorId()]);
            
            if (!state.myId || !state.myUsername) {
                UI.load('Bağlantı Hatası', 'Kullanıcı verisi alınamadı. Hesabınıza giriş yaptığınızdan emin olun.');
                $('tuf-progress-fill').style.background = '#f87171';
                if (!$('tuf-err-btn')) $('tuf-screen-loading').insertAdjacentHTML('beforeend', `<button id="tuf-err-btn" class="tuf-primary-btn tuf-close-btn" style="margin-top:20px;width:auto;">Arayüzü Kapat</button>`);
                return;
            }
            
            const p = window.location.pathname;
            if (p !== `/@${state.myUsername}` && !p.startsWith(`/@${state.myUsername}/`)) {
                UI.load('Güvenlik Protokolü', `İşlemlerin güvenliği için kendi profilinizde olmalısınız. Profilinize (@${state.myUsername}) yönlendiriliyorsunuz...`);
            ...
            if (state.myUsername === CONFIG.CREATOR || (fw || []).some(u => u.username === CONFIG.CREATOR)) {
                ...
                const unlockSystem = () => {
                    ...
                    if (window._tufObserver) window._tufObserver.disconnect();
                    bf.innerHTML = 'Kilit Açıldı'; bf.style.background = '#4ade80'; bf.style.color = '#000';
                    ...
                    if (!state.isUnlocked && $('tuf-root')) {
                        window._tufObserver = new MutationObserver(() => {
                            ...
                            if (o.style.display === 'none' || cs.display === 'none' || cs.visibility === 'hidden') {
                                const list = $('tuf-list');
                                if (list && !list.innerHTML.includes('Güvenlik İhlali')) {
                                    list.innerHTML = '<div class="tuf-empty" style="color:#f87171;">Güvenlik İhlali Tespit Edildi!<br><br>Sayfayı yenileyin.</div>';
                ...
                bf.onclick = async () => {
                    ...
                    im = true;
                    ed.innerHTML = 'Takip onayı alınamadı. Lütfen manuel takip edip doğrulayın.';
                    ed.style.display = 'block';
                    bf.innerHTML = 'Takip Ettim, Doğrula'; bf.disabled = false;
